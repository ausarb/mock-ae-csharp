...
namespace Mattersight.mock.ba.ae.Tests.ProcessingStreams.RabbitMQ.ConsumingStream
{
    ...
    {
        [Scenario]
        public void When_subscribers_event_proccesing_throws_an_exception(StreamConsumer<object> sut, IBasicConsumer consumer, Mock<IModel> channel, ulong deliveryTag = 1234)
        {
            ...
            try
            {
                ...
                "with a failing subscriber"
                    .x(() => sut.Subscribe(x => throw new NotImplementedException()));
                ...
                "It should NACK the message"
                    .x(() => channel.Verify(x => x.BasicNack(deliveryTag, false, true), Times.Once));

                "It should not ACK the message"
                    .x(() => channel.Verify(x => x.BasicAck(It.IsAny<ulong>(), It.IsAny<bool>()), Times.Never()));
            }
            finally
            {
                ctx.Cancel();
